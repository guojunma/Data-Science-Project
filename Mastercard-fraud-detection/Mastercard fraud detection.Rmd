...
We see that most fraud are through online and subscription payment. 
...
(Create a data with the same number of case and controls)
```{r}
...
print(comparison)
```
It seem like all models have a similar performance. Random forest is slightly higher than the others in the AUC value again. 



In this project, I subset the original data so that the case/control is balanced. This improves the performance compared to use the original data. However, this comes with a problem: we throw away a majority of control which also contain a lot of information. Further, the training also highly dependent on which controls are selected. 

Research direction: 
Methods extend the data analysis with unbalanced binary variables: 
 - Generate case samples so that the case/control ratio is equal. This way, We also retain the large samples size. Maybe we can try to generate samples according to papers "Deep knockoff"? 
 - We could also fit the model multiple times and aggregate the result together. Fit the model multiple time for each equal case/control samples. 
